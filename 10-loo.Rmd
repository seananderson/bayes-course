...
- LPD: log (pointwise) predictive density (typically on fitted data)
...
So far, we have calculated ELPD on the data predicted from a model fit to all the data. But, we know this is an overly optimistic perspective on predictive ability for new data. Instead, we can compute log LOO predictive densities, which is typically what we mean by ELPD.
...
Each noise predictor is expected to add 0.5 to the in-sample ELPD and subtract 0.5 from the LOO-ELPD.
