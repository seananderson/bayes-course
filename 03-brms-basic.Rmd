...
```{r}
d <- mutate(d,
  ph_scaled = (ph - mean(ph)) / 2 * sd(ph)
)
```

<!-- We centered the `nutrients` data, which is a binary (0, 1) variable. Why do you think we did that? Does it matter here? -->
...
# see `?geom_histogram()`
ggplot() +
  geom_histogram(data = post, aes(b_ph_scaled, ..density..), 
    bins = 80) +
...
fit2 <- update(fit, formula. = . ~ . +
  nutrients:ph_scaled + nutrients:I(ph_scaled^2))
```
...
Now we can compare the models with LOOIC. We will set `k_threshold = 0.7`. The `k` value is an indication of how well the approximation of leaving out a single data point is expected to be to actually refitting the model and leaving out that data point. The Stan developers recommend 0.7 as a reasonable threshold. By setting the threshold, brms will refit the model successively leaving out any data points that weren't well approximated. This is quite fast here, but could be slow for much more complicated models or models for much larger data sets. 

```{r}
loo1 <- loo(fit, k_threshold = 0.7, moment_match = TRUE)
loo2 <- loo(fit2, k_threshold = 0.7, moment_match = TRUE)
```
