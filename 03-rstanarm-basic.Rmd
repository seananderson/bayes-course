...
Any time we are using rstan (or a package that relies on rstan, such as rstanarm), we can set an R option to use parallel processing with all available cores. This example should run so quickly that it will likely run faster on a single core, so you may choose to skip this or explicitly set it to 1 core.
...
```{r}
d <- readRDS("data-raw/hughes-etal-2018.rds") %>%
...
Let's rescale the predictors. This is important

1. so that we have some idea of what reasonable prior values will be,
2. our coefficients are on a reasonable scale for interpretation and for Stan, and
3. so that we can add a quadratic effect and have one coefficient represent the slope and the other the curvature.

```{r}
d <- mutate(d,
  ph_scaled = arm::rescale(ph),
  nutrients_centered = arm::rescale(nutrients)
)
```
...
The primary way of looking at MCMC chains is as overlaid timeseries:
...
Realistically this is just about as perfect as it gets.
...
Note the use of `as.array()` to convert the output from rstanarm first. `as.array()` maintains the separate chains for the plotting functions.
...
Experiment with the available plotting functions to summarize the posterior probabilities of the parameters in our model. Which do you find most useful? 
...
Why might we prefer 90% or 50% or even 87% credible intervals over the usual 95%? Note that rstanarm and bayesplot default to 90% credible intervals.
...
It's helpful to know that you can extract the priors from an rstanarm model with the `prior_summary()` function. In this case we specified all the priors explicitly, which is a good practice. This function is a good way to check that the priors were interpreted correctly, and is also a good way to discover parameters that you might have forgot to set the priors on.
...
We can extract samples from the linear predictor with the `posterior_linpred()` function. These are samples from the posterior without observation error added on. In other words, these are similar in concept to the confidence interval you would get out of `predict.glm()` or `predict.lm()`.
...
# Summarizing the posterior samples ourselves

One of the beauties of Bayesian statistical models is that we can quantify the probability of nearly any comparison you can imagine. And all you have to do is add, subtract, multiply, or divide the samples. Let's try some examples.

For models fit with rstanarm, we can extract the posterior samples with `as.data.frame()` or `as.matrix()`. Let's use the data frame version. We'll also converted to a tibble, just so that it prints nicely.

```{r}
post <- as.data.frame(fit) %>% as_tibble()
post
```
...
What is the probability that the change in seahare mass is greater in the case where nutrients weren't added? 
...
First we should make sure that the model we just fit isn't giving us gibberish.
...
So this suggests that we should favor the original model that did not have interactions between nutrients and pH.
