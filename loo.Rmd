...
This example builds on content and code from the book "Regression and Other Stories" <https://avehtari.github.io/ROS-Examples/> by Gelman, Hill, and Vehtari.

We're going to use rstanarm instead of brms here just because rstanarm doesn't require us to compile the model each time, so the code here will run much faster.

Let's get some acronyms out of the way first:

- LPD: log (pointwise) predictive density (usually on new data)
- ELPD: expected log (pointwise) predictive density (usually on new data)
- LOO: leave-one-out
...
# The concept of log scores, LPD, and ELPD
...
Compute the average log pointwise posterior density (LPD) in a computationally stable way:
...
Now, calculate the log density for each observation and each MCMC sample:
...
These match. The sum of these log predictive densities are called the 'ELPD': the expected log predictive densities.
...
So far, we have calculated ELPD on the data predicted from a model fit to all the data. But we know this isn't overly optimistic perspective on predictive ability for new data. Instead, we can compute log LOO predictive densities.
...
This approximation is equivalent (but much faster) than this:
...
for (i in 1:n) {
  cat(i, "\n")
  this_dat <- fake[-i, ]
  fit_minus_i <- rstanarm::stan_glm(y ~ x, data = this_dat, seed = 2141, refresh = 0)
...
We work with a regression on a dataset of child IQ sores, mom IQ scores, and other covariates such as whether the mom finished highschool. It's from the Regression and Other Stories book and originally from the National Longitudinal Survey of Youth.
...
ELPD favours the model without random predictors, but the difference isn't large. Presumably we'd pick the simpler model.
...
The basic idea is that excluding one data points is equivalent to subtracting the log density of that point data point (assuming independence) or dividing the density by the density for that one data point. We can use this property to form weights to sample from our existing posterior samples to approximate the posterior if we had dropped that data point. This is a form of "importance sampling".
...
Now weight the MCMC samples by weights of `1/exp(log_dens)`:
...
for (i in 1:n) {
  ...
  sampled_rows <- sample(row_ids, 2000, prob = weights, replace = TRUE)
  lpd_loo1[i] <- log(mean(log_dens[sampled_rows, i]))
}
loo::loo(fit_all)
sum(lpd_loo1) # about the same
```

The only difference with the calculations in the loo package is the package does some smoothing on the weights since they can get noisy.
