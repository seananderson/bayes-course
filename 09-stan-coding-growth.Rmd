...
ggplot(post, aes(age, .value, group = .draw)) + geom_line(alpha = 0.2) +
  geom_point(data = d_wcvi, mapping = aes(age, length), inherit.aes = FALSE, alpha = 0.2)
```
...
Take the above model and modify it to allow for separate k, linf, and t0 by survey region (HS vs. WCVI). Share the observation error SD. Ask whether LOO ELPD favours us estimating separate regions or defining them as one stock (according this one criteria).
