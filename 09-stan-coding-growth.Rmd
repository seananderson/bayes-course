...
- See how we can do posterior (and prior) predictive simulations in a custom Stan model.
...
These are Pacific Cod age and length data from two synoptic trawl surveys in BC.
...
We will fit a von Bertalanffy growth model to them. Here's a simple version:
...
fit1 <- stan("vb/vb_norm.stan", data = dat, iter = 1000, chains = 4, seed = 9129)
```

Print the parameters of interest:

Question: why am I specifying the parameters explicitly here?
...
We can calculate ELPD (expected log predictive density) of a leave-one-out approximation because we included `log_lik` in our `generated quantities` section. It's not all that useful yet though, since we've only fit one model.
...
fit2 <- stan("vb/vb_norm_regions.stan", data = dat2, iter = 1000, chains = 4, seed = 9129)
```
